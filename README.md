...
Sample output from the Doppelgägner system is in the [description](#The-Program) of the **doppelganger_finder.py** script.
...
The following 12 files and 3 folders are included in this repository:
  ...
  3.	`image_data/` - A folder containing the user images and imdb/wikipedia images (for size purposes a download link is included, these images are necessary to run **doppelganger_finder.py**)
...
I selected the FaceNet model over the Visual Geometry Group (VGG16) model for three reasons: The first reason is that FaceNet achieved better results on benchmark datasets; The second reason is that the embedding vector produced by FaceNet is only 128 elements long while the vector from the VGG model is 2,622 elements long. The VGG vectors would have been too large to store in the .csv format that I use. The VGG architecture can be found here: [Deep Face Recognition](http://www.robots.ox.ac.uk/~vgg/publications/2015/Parkhi15/parkhi15.pdf) (*by Omkar M. Parkhi, Andrea Vedaldi, and Andrew Zisserman*). The third reason is that FaceNet is optimized to produce face embeddings. Previous models output a class that a vector belongs to and face embeddings can be extracted in earlier layers. FaceNet's final layer is the embedding vector itself which is optimized for the purposes of similarity comparison.
...
![Fred Astaire FaceNet Embedding](/image_data/Images_for_ReadMe/Fred_Astaire_FaceNet_Embedding.jpg)
...
2. Serengil, Sefik Ilkin. “Celebrity Look-Alike Face Recognition with Deep Learning in Keras.” Celebrity Look-Alike Face Recognition with Deep Learning in Keras, 5 May 2019, [https://sefiks.com/2019/05/05/celebrity-look-alike-face-recognition-with-deep-learning-in-keras/](https://sefiks.com/2019/05/05/celebrity-look-alike-face-recognition-with-deep-learning-in-keras/).
